...
console.log('-------------------------------------');
//anti raid / anti nuke bot coded by itsweekly >:D
...
`);
console.log('-------------------------------------');
console.log('[CREDITS]: made by itsweekly#5901')
console.log('-------------------------------------');
console.log(`
 █████╗ ███╗   ██╗████████╗██╗      ██████╗  █████╗ ██╗██████╗ 
██╔══██╗████╗  ██║╚══██╔══╝██║      ██╔══██╗██╔══██╗██║██╔══██╗
...
client.on('ready', () => {
	...
	console.log('-------------------------------------');
	client.user.setActivity('anti raid by itsweekly :D', { 
		type: 'WATCHING'
	});
});
    ...
    if(person > limit - 1) {
      if (punish === "ban") {
        role.guild.members.ban(user.id).then(bruhmoment => {
          ...
          .setFooter(role.guild.name + " | made by itsweekly#5901", role.guild.iconURL())
    ...
    if(person > limit - 1) {
      if (punish === "ban") {
        channel.guild.members.ban(user.id).then(hshshshs => {
          ...
          .setTitle("**Anti-Raid**")
          .setThumbnail(user.displayAvatarURL({ dynamic: true }))
          .setFooter(channel.guild.name + " | made by itsweekly#5901", channel.guild.iconURL())
          .addField("User", user.tag)
    ...
    if(person > limit - 1) {
      if (punish === "ban") {
        member.guild.members.ban(user.id).then(lolxdbruh => {
          ...
          .setTitle("**Anti-Raid**")
          .setThumbnail(user.displayAvatarURL({ dynamic: true }))
          .setFooter(member.guild.name + " | made by itsweekly#5901", member.guild.iconURL())
          .addField("User", user.tag)
